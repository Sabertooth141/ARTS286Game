//get inputs
var _key_left = global.starting ? false : -keyboard_check(vk_left);
var _key_right = global.starting ? false : keyboard_check(vk_right);
var _key_down = global.starting ? false : keyboard_check(vk_down);
var _key_jump = global.starting ? false : keyboard_check_pressed(vk_space);
...
if (!global.commited) {
	...
	if (place_meeting(x + hsp, y, [tilemap, obj_pod])) {
		hsp = 0;
	}
	...
	// vertical collision
	if (place_meeting(x, y + 10, [tilemap, obj_pod])) {
		is_onground = true;
	} else {
		is_onground = false;
	}
